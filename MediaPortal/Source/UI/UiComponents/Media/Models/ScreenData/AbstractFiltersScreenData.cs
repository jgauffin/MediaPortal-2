...
namespace MediaPortal.UiComponents.Media.Models.ScreenData
{
  public abstract class AbstractFiltersScreenData<T> : AbstractScreenData where T : FilterItem, new()
  {
      ...
      try
      {
        ...
        try
        {
          ...
          else
          {
            ...
            AbstractScreenData currentScreen = _navigationData.AvailableScreens.FirstOrDefault(screen => screen.MenuItemLabel == MenuItemLabel);
            ICollection<AbstractScreenData> remainingScreens = new List<AbstractScreenData>(_navigationData.AvailableScreens.Where(screen => screen != currentScreen));
            foreach (FilterValue filterValue in fv)
            {
              ...
              T filterValueItem = new T
              {
                ...
                Command = grouping ? 
                  ...
                  new MethodDelegateCommand(() => NavigateToSubView(subVS, currentScreen, remainingScreens))
              };
    ...
    protected void NavigateToSubView(ViewSpecification subViewSpecification, AbstractScreenData currentScreen, ICollection<AbstractScreenData> remainingScreens)
    {
      _navigationData.StackAutonomousNavigationContext(subViewSpecification, currentScreen, remainingScreens, GetNavbarDisplayLabel(subViewSpecification));
    }
  }
}